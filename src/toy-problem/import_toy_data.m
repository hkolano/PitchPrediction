...
%% Data import and processing
    sequence_data = import_toy_training_data("data/toy-data-with-desv");
    ...
    % remove the outliers from the dataset, from highest to lowest index
    for idx = length(bad_ids):-1:1
        sequence_data(bad_ids(idx)) = [];
        waypoint_data(:, bad_ids(idx)) = [];
    end
    
    % Move the des_vels data one step ahead and delete the last frame in
    % the sequence
    for n = 1:numel(sequence_data)
        sequence_data{n}(7:8,1:end-1) = sequence_data{n}(7:8,2:end);
        sequence_data{n}(:,end) = [];
    end
    ...
    7-8: desired velocities(J1, J2) from generated trajectory NORMALIZED
    9: time step of data collection (constant)
    10-11: known starting positions (constant)
    12-13: goal end positions (constant)
    14-15: known starting velocities (constant)
    16-17: goal end velocities (constant)
    X(:,1:9) are fed to the LSTM, while X(:,9:end) are sent directly to the
    FCN.
    %}
...
%     
    outputFile2 = fullfile("data/toy-data-matlab", 'TestandTrainData_WithDesVels_080122.mat');
