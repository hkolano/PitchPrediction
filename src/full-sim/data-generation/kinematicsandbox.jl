...
# function simple_ik_iterator(state)
    ...
    for t in range(0, stop=1, step=Δt)
        ...
        des_veh_twist_body = RigidBodyDynamics.Twist(body_frame, base_frame, body_frame, SVector{3}(ζ[1:3]), SVector{3}(ζ[4:6])) 
        des_veh_twist_space = transform(des_veh_twist_body, transform_to_root(new_state, vehicle_body))
