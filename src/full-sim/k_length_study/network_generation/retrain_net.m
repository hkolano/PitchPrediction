%{
Generates the autoregressive networks by modifying the single step network
generated by generate_single_step_autoreg.net. 

Interrupts training after each mini-batch to make new predictions. This
process runs very slowly -- definitely use the HPC with a GPU or a laptop
with a GPU.

It should plot training progress as it runs, and save the network every 5
iterations so it can be restarted if it fails. 

Currently does predictions of 0.5s, 1s, 2s, 3s, and 4s. 
